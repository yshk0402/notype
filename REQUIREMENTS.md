...
> 本書は要件定義のラフ整理です。未確定は **{{TODO}}** として残します。
...
* 単一インスタンス前提。
* CLI オプションの実装方式：{{TODO: D-Bus / Unix ソケット / PID ファイル等から選定}}
* デスクトップエントリ（.desktop）の提供：{{TODO: 提供する/しない}}
* 配布方式：{{TODO: Flatpak / AppImage / deb など方針確定}}
...
* モデル配布の扱い：{{TODO: リポジトリ同梱 / リリース資産 / 初回取得など方針}}
...
* API キー保管：{{TODO: GNOME Keyring（Secret Service）を優先}}（平文保存を避ける）
...
* FR-12：モデルは少なくとも small / medium を扱える（初期は medium）
...
* 入力注入のフォールバック：{{TODO: wtype 失敗時に ydotool 等をどう扱うか（MVP で同梱 or 後回し）}}
...
* NFR-03：10 秒程度の発話を許容範囲で処理できる（設定で調整可能）
